...
**▶️ [Assista à apresentação do projeto]([https://drive.google.com/drive/folders/1RSx9I6UF9Dm1Xm_DMWTtKhCnytWAfC2P](https://drive.google.com/file/d/16NHnRmOOJiNBwtRK3UuGrlM3nbQYBvQt/view?usp=sharing))**
...
A alfabetização na idade certa é um dos fundamentos do desenvolvimento educacional, social e econômico de um país. Para enfrentar esse desafio, o Brasil instituiu o Compromisso Nacional Criança Alfabetizada, cujo acompanhamento é feito pelo **Indicador Criança Alfabetizada** (INEP): o percentual de crianças do 2º ano do ensino fundamental consideradas alfabetizadas, definido pela proficiência mínima de 743 pontos na escala Saeb.

Os resultados nacionais avançam (55,9% em 2023, 59,2% em 2024 e 66,0% em 2025, na rede pública), mas as médias encobrem desigualdades territoriais expressivas: entre municípios e redes de ensino, as taxas variam de aproximadamente 2% a 100%. Compreender essas diferenças exige integrar dados que hoje estão dispersos em fontes distintas (o indicador por município e UF, as metas pactuadas em cada esfera, os dados territoriais e os microdados de cerca de 4 milhões de alunos avaliados por ano), e a qualidade dessas informações condiciona a capacidade dos gestores públicos de priorizar ações e recursos.

Para maiores detalhes sobre o contexto deste trabalho, incluindo os impactos econômicos da não alfabetização e a metodologia do indicador, acesse [docs/contexto_do_problema.md](docs/contexto_do_problema.md).
...
├── docs/
│   ├── contexto_do_problema.md            # contexto detalhado (versão resumida na seção 1)
│   ├── dicionario_dados.md
...
| Desenvolvimento (`notebooks/`) | Produção (`src/`) |
|---|---|
| `desenv_00_levantamento_fontes_dados.py` | (sem par: levantamento de fontes) |
| `desenv_01_ingestao_batch.ipynb` | `ingestion/prod_01_ingestao_batch.py` |
| `desenv_02_ingestao_streaming.ipynb` | `ingestion/prod_02_ingestao_streaming.py` |
