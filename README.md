...
Por esse motivo, a arquitetura trata as tabelas consolidadas como fonte oficial do histórico, ingeridas por batch, e utiliza os microdados como base da simulação de streaming: eventos no nível do aluno representam a chegada contínua de resultados da avaliação de 2025 e são agregados pela pipeline para produzir uma visão atualizada do indicador, identificada como estimativa própria, sem substituir os valores oficiais consolidados. Ingerir as demais fontes por streaming adicionaria complexidade e custo sem benefício, uma vez que sua atualização é, por natureza, esporádica e consolidada.
